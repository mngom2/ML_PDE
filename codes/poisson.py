...
def main(argv):
    ...
    sol_der = tf.placeholder(tf.float64, [None, 2])
    sol_der2 = tf.placeholder(tf.float64, [None, 2])
    ...
    for i in range(NUM_INPUTS):
        # sum_of_second_derivatives += value_int * sol_der2[i]
        sum_of_second_derivatives_sensor += grad_grad_sensor[i] #+ grad_grad_sensor[1]
...
#    loss_sensor_bou = tf.square(value_sensor)
    ...
    loss = tf.sqrt(tf.reduce_mean(loss_int))
    sensor_loss = tf.sqrt(tf.reduce_mean(loss_int)  + tf.reduce_mean(loss_sensor_int))
    ...
    with tf.Session() as sess:
        ...
        der_sol = problem.der_velocity(int_draw)
        der_sol = np.reshape(np.array(der_sol), (BATCHSIZE, 2))
        
        der2_sol = problem.second_der_velocity(int_draw)
        der2_sol = np.reshape(np.array(der2_sol), (BATCHSIZE, 2))
        ...
        if(SENSOR_DATA):
            ...
            train_scipy_sensor.minimize(sess, feed_dict={sol_int:f, int_var:int_draw,  sensor_var: sensor_points})
        else:
            train_scipy.minimize(sess, feed_dict={sol_int:f, sol_der: der_sol, sol_der2:der2_sol,  int_var:int_draw})
