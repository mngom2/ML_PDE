...
def ngom_init(in_dim, out_dim):
    pi = np.pi
    ngom_stddev1 = np.sqrt((pi*pi/3.0 - .5 - 4.0/(pi*pi))*6/(in_dim * pi*pi))    #np.sqrt(2/(in_dim + out_dim))
    ngom_stddev2 = np.sqrt((pi*pi/3.0 )/(out_dim * (pi*pi/3.0 - 4.0/(pi*pi))))
    w01 = tf.Variable(tf.truncated_normal([in_dim, out_dim], stddev=ngom_stddev1, dtype=tf.float64), dtype=tf.float64)
...
def ngom_bias_ini(out_dim):
    pi = np.pi
    biases= []
    stddev_bias = pi/np.sqrt(12.0)
    b1 = tf.Variable(tf.random_uniform([out_dim] ,minval = -pi/2.0, maxval = pi/2.0,dtype=tf.float64), dtype=tf.float64)
...
class neural_network:
    def __init__(self,
        ...
        for i in range(0, self.number_of_layers):
            if i == 0:
                self.weights[self.name+'0'] = tf.get_variable(self.name + 'weight_' + str(0), initializer=self.weight_initialization[0], dtype=tf.float64)
                #else:
            ...
            self.biases[self.name+str(i)] = tf.get_variable(self.name + 'bias_' + str(i), initializer=self.bias_initialization[0], dtype=tf.float64)
        
        self.weights[self.name+str(self.number_of_layers)] =  tf.get_variable(self.name+'weight_' + str(self.number_of_layers), initializer=self.weight_initialization[1], dtype=tf.float64)
        self.biases[self.name+str(self.number_of_layers)] =tf.get_variable(self.name+'bias_' + str(self.number_of_layers), initializer=self.bias_initialization[1], dtype=tf.float64)
    ...
    def value(self, input_var):
        ...
        layer2 = tf.cos(tf.add(tf.matmul(input_var, self.weights[self.name+'0']), self.biases[self.name+'0']))
        ...
        print(self.biases[self.name+'0'].shape)
        # sleep
